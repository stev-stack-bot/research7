...
from src.run_lighter_pipeline_v3 import (
    ...
    compute_bar_features_streaming, simulate_lighter_backtest_v3
)
...
def train_lighter_meta_labeler_v4(df, train_events):
...
def simulate_lighter_backtest_v4(df, events_df, maker_fee=0.0, taker_fee=0.0, slippage=0.0, meta_model=None, prob_thresh=0.5):
    if events_df.empty:
        return {
            "pnl_df": pd.DataFrame(),
            "metrics": {
...
def apply_lighter_triple_barriers_asymm(df, book_times, best_bids, best_asks, trade_events, asymm_params, latency_ms=300, maker_latency_ms=200):
    events = []
...
def run_market_backtest(symbol):
    ...
    # 5. Apply Asymmetric Triple Barrier Exits
    print("Applying Asymmetric Triple Barrier Method exits...")
    # Standard: 300ms taker latency, 200ms maker latency
    std_events = apply_lighter_triple_barriers_asymm(
    ...
    prem_events = apply_lighter_triple_barriers_asymm(
        df_features, book_times, best_bids, best_asks, trades,
    ...
    if not std_events.empty:
        ...
        if len(std_train_purged) > 0:
            std_model = train_lighter_meta_labeler_v4(df_features, std_train_purged)
    ...
    if not prem_events.empty:
        ...
        if len(prem_train_purged) > 0:
            prem_model = train_lighter_meta_labeler_v4(df_features, prem_train_purged)
    ...
    std_baseline_res = simulate_lighter_backtest_v4(df_features, std_val_events, maker_fee=0.0, taker_fee=0.0, slippage=0.00005)
    std_meta_res = simulate_lighter_backtest_v4(df_features, std_val_events, maker_fee=0.0, taker_fee=0.0, slippage=0.00005, meta_model=std_model, prob_thresh=0.5)
    
    # Premium costs: 0.4 bps maker, 2.8 bps taker, 0.2 bps slippage
    prem_baseline_res = simulate_lighter_backtest_v4(df_features, prem_val_events, maker_fee=0.00004, taker_fee=0.00028, slippage=0.00002)
    prem_meta_res = simulate_lighter_backtest_v4(df_features, prem_val_events, maker_fee=0.00004, taker_fee=0.00028, slippage=0.00002, meta_model=prem_model, prob_thresh=0.5)
...
def main():
    ...
    if results:
        md_report = """# Robust Asymmetric Baseline Backtesting Report (14-Hour Dataset)

This report presents backtesting results with the new robust asymmetric parameters applied to the baseline, combined with the 14-feature meta-labeler.
...
"""
        ...
        report_path = "/kaggle/research7/multi_pair_14h_report_v4.md"
