...
def compute_bar_features_streaming_v7(bars_df, book_files, trades, z_window=100):
    ...
    # Kyle's Lambda (rolling price impact)
...
def apply_lighter_triple_barriers_v7(df, book_times, best_bids, best_asks, best_bid_szs, best_ask_szs, trade_events, asymm_params, latency_ms=300, maker_latency_ms=200, maker_fee=0.0, taker_fee=0.0, slippage=0.0):
    events = []
    ...
    for idx, row in df.iterrows():
        ...
        resting_size = best_bid_szs[book_idx] if direction == 1 else best_ask_szs[book_idx]
        
        # Check fill probability with Touch Volume clearance and Post-Only Pull
        ...
        for k in range(first_trade_idx, len(trade_times)):
            ...
            # Pull order if velocity exceeds 10.0 (toxic volume filter)
            if velocity_vals[current_bar_idx] > 10.0:
                break
...
def run_market_backtest_v7(symbol):
    ...
    print(f"\n=======================================================")
    print(f" PROCESSING MARKET (V7 QUEUE MODEL): {symbol} (Market ID: {mid})")
    print(f"=======================================================")
    ...
    df_features, book_times, best_bids, best_asks, best_bid_szs, best_ask_szs = compute_bar_features_streaming_v7(
        bars_df, book_files, trades, z_window=100
    )
    
    print("Applying Asymmetric Triple Barrier Method exits (V7 Queue Dynamics + Post-Only Pull)...")
    # Standard: 300ms latency, 0% fees, 0.5 bps slippage (only on taker exits)
    std_events = apply_lighter_triple_barriers_v7(
        df_features, book_times, best_bids, best_asks, best_bid_szs, best_ask_szs, trades,
    ...
    prem_events = apply_lighter_triple_barriers_v7(
        df_features, book_times, best_bids, best_asks, best_bid_szs, best_ask_szs, trades,
    ...
    print("\n=======================================================")
    print(f"            LIGHTER PERFORMANCE (V7 QUEUE MODEL) ({symbol})")
    print("=======================================================")
...
def main():
    ...
    results = {}
    for sym in symbols:
        res = run_market_backtest_v7(sym)
        if res:
            results[sym] = res
            
    if results:
        md_report = """# Queue-Position & Pull-Signal Backtesting Report (V7)

This report presents backtesting results with passive maker limit orders, touch volume queue dynamics, and velocity-based cancel/pull filters.
...
"""
        ...
        report_path = "/kaggle/research7/multi_pair_61h_report_v7.md"
        with open(report_path, "w") as f:
            f.write(md_report)
        print(f"\nWritten V7 Queue Maker summary report to: {report_path}")

        with open("/kaggle/research7/data/v7_maker_scorecard.json", "w") as f:
